...
<head>
    ...
    <title>Supprimer une image de l'habitat</title>
    <!-- Bootstrap 5.1 CSS -->
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        try {
            ...
            if ($image->deleteHabitatImage($image_id)) {
                header('Location: detail_habitat.php?id=' . $_POST['id'] . '&message=suppressionOk');
            } else {
    ...
    // Récupération des images de l'habitat
    $habitat_id = (int) $_GET['id']; // Convertir en entier
    $resultatImage = $habitat->getHabitatImages($habitat_id);
    ?>
    <div class="container">
        <h1>Supprimer une image de l'habitat <?php echo htmlspecialchars($_GET['nom']); ?></h1>
        <form action="" method="post">
            ...
            <button class="btn btn-primary mt10perso"><a class="lien"
                    href="detail_habitat.php?id=<?php echo htmlspecialchars($_GET['id']); ?>">Retour sur la fiche de
                    l'habitat</a></button>
        </form>
    </div>
